...
class AuthServices {
  Future<ServerResponse<LoginResponseModel>> signInWithEmailAndPassword(
  ...
  Future<ServerResponse<CreateAccountResponseModel>>
      createUserWithEmailAndPassword(BuildContext context, String firstName,
    ...
    try {
      ...
      Map<String, dynamic> response = await GraphqlApi.mutation(
          createAccountMutation, _requestVariables.toJson());
      CreateAccountResponseModel responseModel =
      ...
      if (responseModel.customerCreate!.customer != null) {
        ...
        return ServerResponse.error(
            responseModel.customerCreate!.customerUserErrors!.first.message);
      }
    } on Exception catch (e) {
      controller.setSignupLoading(false);
      return ServerResponse.error(e.toString());
  ...
  Future<ServerResponse<SendPasswordResetResponseModel>> sendPasswordResetEmail(
      BuildContext context, String email) async {
    var controller = Provider.of<AuthController>(context, listen: false);
    try {
      controller.setForgetPasswordLoading(true);
      SendPasswordResetRequestModel _requestVariables =
          SendPasswordResetRequestModel(email: email);
      Map<String, dynamic> response = await GraphqlApi.mutation(
          sendPasswordResetMutation, _requestVariables.toJson());
      SendPasswordResetResponseModel responseModel =
          SendPasswordResetResponseModel.fromJson(response);
      controller.setForgetPasswordLoading(false);
      if (responseModel.customerRecover!.customerUserErrors!.isEmpty &&
          responseModel.customerRecover!.userErrors!.isEmpty) {
        return ServerResponse.completed(responseModel);
      } else {
        return ServerResponse.error(
            responseModel.customerRecover!.customerUserErrors!.first.message);
      }
    } on Exception catch (e) {
      controller.setForgetPasswordLoading(false);
      return ServerResponse.error(e.toString());
